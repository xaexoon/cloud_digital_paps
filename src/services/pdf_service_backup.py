...
class PapsPdfGenerator:
    ...
    def _draw_bar_charts(self, c: canvas.Canvas, data: StudentData):
        ...
        for bar_y_coord, value, max_val, reverse, unit in bar_config:
            ...
            c.roundRect(pdf_x, pdf_y - pdf_bar_height / 2, fill_width, pdf_bar_height, pdf_bar_height / 4, fill=1,
                        stroke=0)
    ...
    def _draw_text_values(self, c: canvas.Canvas, data: StudentData):
        ...
        c.drawCentredString(x, y, self._get_bmi_label(data.grade_bmi))
    ...
    def _draw_radar_chart(self, c: canvas.Canvas, data: StudentData):
        ...
        m_grade = data.grade_rolling_up if data.muscle_exercise == "rollingUp" else min(data.grade_grip_left,
                                                                                        data.grade_grip_right)
    ...
    def _get_bmi_label(self, grade):
        return {1: "정상", 2: "과체중", 3: "경도비만", 4: "중등도비만", 5: "고도비만"}.get(grade, str(grade))
